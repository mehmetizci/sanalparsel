...
export default function EnvironmentPage({ params }: { params: { id: string } }) {
  ...
  const handleFetchFromOsm = async () => {
    ...
    // Skip if same parcel - use cached data (but still log)
    if (nearbyParcelKey === parcelKey && pois.length > 0) {
      console.log("[Environment] Same parcel, using cached POIs");
      console.log("[Environment] Cached POI Debug:", {
        radius: "from_cache",
        requestedCategories: ["market", "pharmacy", "hospital", "school", "transport", "cafe", "restaurant"],
        rawOsmResultCount: pois.length,
        normalizedPoiCount: pois.length,
        filteredPoiCount: pois.length,
        displayedPoiCount: pois.length,
        selectedPoiCount: pois.filter(p => p.selected).length
      });
      setItems(pois.map((p, i) => poiToEnvironmentItem(p, id, i)));
      return;
    }
    ...
    try {
      ...
      console.log("[Environment] POI Debug:", {
        radius: "1500m (initial) / 3000m / 5000m (adaptive)",
        requestedCategories: ["market", "pharmacy", "hospital", "school", "transport", "cafe", "restaurant"],
        rawOsmResultCount: apiPois.length,
        normalizedPoiCount: apiPois.length,
        filteredPoiCount: apiPois.length,
        displayedPoiCount: Math.min(apiPois.length, 7),
        selectedPoiCount: apiPois.filter(p => p.selected).length
      });
      ...
      if (apiPois.length === 0) {
        setError({
          type: "OVERPASS_ERROR",
          message: "Bu parsel çevresinde uygun çevre bilgisi bulunamadı.",
        });
        return;
      }
      ...
      // Update items for UI (limit to max 7)
      ...
      if (err instanceof POIServiceError) {
        switch (err.code) {
          case "MISSING_COORDS":
            ...
            break;
          case "TIMEOUT":
            setError({ type: "TIMEOUT", message: "Çevre bilgileri alınamadı. Lütfen tekrar deneyin." });
            break;
          case "OVERPASS_ERROR":
            setError({ type: "OVERPASS_ERROR", message: "Çevre bilgileri alınamadı. Lütfen tekrar deneyin." });
            break;
          case "API_ERROR":
            setError({ type: "OVERPASS_ERROR", message: "Çevre bilgileri alınamadı. Sunucular meşgul veya erişim sınırlandırıldı." });
            break;
          case "NETWORK_ERROR":
        ...
        setError({ type: "UNKNOWN", message: "Çevre bilgileri alınamadı. Lütfen tekrar deneyin." });
      }
    } finally {
